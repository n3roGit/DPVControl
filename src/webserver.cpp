...
<body>
            ...
            <div class="section">
                ...
                <div class="settings-group">
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        <input type="range" id="lampLevelSlider" min="0" max="2" value="0" 
                               style="width: 100%; height: 6px;" 
                               oninput="updateLampLevel(this.value)" onchange="setLampLevel(this.value)">
                        <div id="lampLevelLabels" style="display: flex; justify-content: space-between; font-size: 12px; color: #b0b0b0; margin-top: 5px;">
                            <!-- Labels will be dynamically generated by updateLampSliderInterface() -->
                        </div>
                    </div>
    ...
    <script>
        ...
        function updateLampLevel(value) {
            ...
            const levelName = levelNames[currentLampLevel] || 'OFF';
            document.getElementById('remoteLampStatus').textContent = levelName;
            
            const lampLevelNameEl = document.getElementById('lampLevelName');
            if (lampLevelNameEl) {
                lampLevelNameEl.textContent = levelName;
            }
        }
        ...
        function updateLampSliderInterface() {
            ...
            const labelContainer = document.getElementById('lampLevelLabels');
            
            if (!slider || !labelContainer) {
                console.error('Lamp slider elements not found');
                return;
            }
            
            // Update slider max value - maxLevels means we have 0,1,2,...,maxLevels (total = maxLevels+1 states)
            ...
            // Generate new labels: For maxLevels=2 -> ['OFF', 'Level 1', 'MAX'] 
            const levelNames = generateLevelNames(maxLevels);
            labelContainer.innerHTML = '';
            
            // Create label spans evenly distributed
            ...
            updateLampLevel(currentLampLevel);
            
            console.log(`Updated lamp slider interface: max=${maxLevels}, levels=[${levelNames.join(', ')}]`);
            console.log(`Slider states: 0=OFF, 1-${maxLevels}=brightness levels`);
        }
