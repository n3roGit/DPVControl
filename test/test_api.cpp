...
void test_status_api_uptime_fields() {
    // Test that status API includes uptime fields (prevents bug where uptime was missing)
    ...
    // Check that uptime fields exist (this would have caught the missing uptime bug)
...
void test_status_api_all_required_fields() {
    ...
    const char* requiredFields[] = {
        "status", "uptime", "totalUptime", "motor", 
        "lamp", "beeper", "erpm", "leftButton", "rightButton"
    };
...
void test_api_responses_are_valid_json() {
    // Test that all API endpoints return valid JSON (prevents malformed responses)
